...
func (uq *UserQuery) QueryTalents() *TalentQuery {
	query := &TalentQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QuerySlackAppInstalls chains the current query on the "slack_app_installs" edge.
func (uq *UserQuery) QuerySlackAppInstalls() *SlackAppInstallQuery {
	query := &SlackAppInstallQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) QueryTalentCollections() *TalentCollectionQuery {
	query := &TalentCollectionQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) QueryFileUploads() *FileUploadQuery {
	query := &FileUploadQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) WithTalents(opts ...func(*TalentQuery)) *UserQuery {
	query := &TalentQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withTalents = query
	return uq
}

// WithSlackAppInstalls tells the query-builder to eager-load the nodes that are connected to
// the "slack_app_installs" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithSlackAppInstalls(opts ...func(*SlackAppInstallQuery)) *UserQuery {
	query := &SlackAppInstallQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withSlackAppInstalls = query
	return uq
}
...
func (uq *UserQuery) WithTalentCollections(opts ...func(*TalentCollectionQuery)) *UserQuery {
	query := &TalentCollectionQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withTalentCollections = query
	return uq
}
...
func (uq *UserQuery) WithFileUploads(opts ...func(*FileUploadQuery)) *UserQuery {
	query := &FileUploadQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withFileUploads = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withTalents; query != nil {
		...
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Talents = []*Talent{}
		}
		query.Where(predicate.Talent(func(s *sql.Selector) {
			s.Where(sql.InValues(user.TalentsColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			...
			node.Edges.Talents = append(node.Edges.Talents, n)
		}
	}

	if query := uq.withSlackAppInstalls; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[uuid.UUID]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.SlackAppInstalls = []*SlackAppInstall{}
		}
		query.Where(predicate.SlackAppInstall(func(s *sql.Selector) {
			s.Where(sql.InValues(user.SlackAppInstallsColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			...
			node.Edges.SlackAppInstalls = append(node.Edges.SlackAppInstalls, n)
		}
	}
	...
	if query := uq.withTalentCollections; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[uuid.UUID]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
		...
		query.Where(predicate.TalentCollection(func(s *sql.Selector) {
			s.Where(sql.InValues(user.TalentCollectionsColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
	...
	if query := uq.withFileUploads; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[uuid.UUID]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.FileUploads = []*FileUpload{}
		}
		query.Where(predicate.FileUpload(func(s *sql.Selector) {
			s.Where(sql.InValues(user.FileUploadsColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
